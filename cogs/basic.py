...
class Greetings(commands.Cog):
    ...
    async def hello(self, ctx):
        await ctx.send(
            f"Hello, {ctx.author.mention}! I'm Xander, nice to meet you :smile:"
        )
...
class Settings(commands.Cog):
    ...
    async def dob(self, ctx):
        await ctx.send(
            "I was born on Christmas' Day, 2020, and reborn on 2nd August, 2023"
        )

    @commands.command(aliases=['git'])
    async def github(self, ctx):
        await ctx.send("https://github.com/XanderWatson/XanderGPT")
